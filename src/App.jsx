...
// ============================================================
//  LISTING FORM MODAL - 3-step form to post real listings
// ============================================================
const CITIES = ["Toronto, ON","Vancouver, BC","Montreal, QC","Calgary, AB","Ottawa, ON","Edmonton, AB","Winnipeg, MB","Hamilton, ON","Halifax, NS"]
const AMENITIES = ["In-unit Laundry","Gym","Parking","Pet Friendly","Balcony","Backyard","Dishwasher","Air Conditioning","Hardwood Floors","Storage Locker","Rooftop Access","Pool","Concierge","EV Charging"]

function SI({value, onChange, options, placeholder}) {
  return (
    <select value={value} onChange={onChange} style={{width:"100%",padding:"11px 14px",borderRadius:T.rsm,fontSize:13,color:value?T.text:T.muted,background:T.bg,border:`1.5px solid ${T.border}`,outline:"none",cursor:"pointer"}}>
...
function FL({label, req}) {
  return <label style={{display:"block",fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:1,color:T.muted,marginBottom:5}}>{label}{req&&<span style={{color:T.accent,marginLeft:3}}>*</span>}</label>
}
...
function ListingFormModal({listing, onClose, onSaved}) {
  ...
  const [step, setStep] = useState(1)
  const blank = {title:"",description:"",address:"",city:"",postal_code:"",price_per_person:"",total_rent:"",beds:"",baths:"",sqft:"",available_spots:"",min_income_required:"",property_type:"apartment",lease_term:"12",rent_controlled:false,pets_allowed:false,smoking_allowed:false,furnished:false,amenities:[],status:"active"}
  const [form, setForm] = useState(listing ? {...blank,...listing,amenities:listing.amenities||[]} : blank)
  const [errs, setErrs] = useState({})
  ...
  async function save() {
    ...
    try {
      const payload = {
        ...
        total_rent: parseFloat(form.total_rent) || (parseFloat(form.price_per_person) * parseInt(form.beds)) || 0,
        beds: parseInt(form.beds) || 0,
        baths: parseFloat(form.baths) || 0,
        sqft: parseInt(form.sqft) || 0,
        available_spots: parseInt(form.available_spots) || 1,
        min_income_required: parseFloat(form.min_income_required) || (parseFloat(form.price_per_person) * 3) || 0,
      ...
      setToast(listing ? "Listing updated!" : "Listing published!")
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:500,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      ...
      <div className="su" style={{position:"relative",background:T.white,borderRadius:`${T.rlg} ${T.rlg} 0 0`,width:"100%",maxWidth:560,maxHeight:"92vh",overflowY:"auto",zIndex:1}}>
        <div style={{display:"flex",justifyContent:"center",padding:"10px 0 0"}}><div style={{width:34,height:4,borderRadius:2,background:T.border}}/></div>
        <div style={{padding:"14px 20px 0",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            <div style={{fontFamily:"'Instrument Serif',serif",fontSize:20}}>{listing ? "Edit Listing" : "New Listing"}</div>
            <div style={{color:T.muted,fontSize:12,marginTop:2}}>Step {step}/{steps.length} — {steps[step-1]}</div>
          </div>
          <button onClick={onClose} style={{background:T.bg,border:"none",width:30,height:30,borderRadius:"50%",cursor:"pointer",fontSize:14,color:T.sub,display:"flex",alignItems:"center",justifyContent:"center"}}>x</button>
        </div>

        <div style={{display:"flex",padding:"0 20px",gap:6,marginBottom:16}}>
          {steps.map((s,i) => (
            <div key={s} onClick={() => setStep(i+1)} style={{flex:1,cursor:"pointer"}}>
              <div style={{height:3,borderRadius:99,background:i+1<=step?T.accent:T.border,transition:"background .3s"}}/>
              <div style={{fontSize:10,fontWeight:600,color:i+1===step?T.accent:T.muted,marginTop:4}}>{s}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{padding:"0 20px 100px"}}>
          {step===1 && (
            <div className="ai" style={{display:"flex",flexDirection:"column",gap:14}}>
              <div><FL label="Listing Title" req /><Inp value={form.title} onChange={e=>set("title",e.target.value)} placeholder="e.g. Bright 2BR Condo Near Subway" icon="🏠" error={errs.title}/></div>
              <div><FL label="Street Address" req /><Inp value={form.address} onChange={e=>set("address",e.target.value)} placeholder="123 Main St" icon="📍" error={errs.address}/></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div><FL label="City" req /><SI value={form.city} onChange={e=>set("city",e.target.value)} options={CITIES} placeholder="Select city..."/>{errs.city&&<p style={{color:T.red,fontSize:11,marginTop:4}}>Required</p>}</div>
                <div><FL label="Postal Code"/><Inp value={form.postal_code} onChange={e=>set("postal_code",e.target.value)} placeholder="M5V 2T6" icon="📮"/></div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div><FL label="Type"/><SI value={form.property_type} onChange={e=>set("property_type",e.target.value)} options={[{value:"apartment",label:"Apartment"},{value:"condo",label:"Condo"},{value:"house",label:"House"},{value:"townhouse",label:"Townhouse"},{value:"basement",label:"Basement"},{value:"studio",label:"Studio"}]}/></div>
                <div><FL label="Lease"/><SI value={form.lease_term} onChange={e=>set("lease_term",e.target.value)} options={[{value:"1",label:"Month-to-Month"},{value:"6",label:"6 Months"},{value:"12",label:"12 Months"},{value:"24",label:"24 Months"}]}/></div>
              </div>
              <div><FL label="Description"/><textarea value={form.description} onChange={e=>set("description",e.target.value)} rows={4} placeholder="Describe your property..." style={{width:"100%",padding:"11px 13px",borderRadius:T.rsm,border:`1.5px solid ${T.border}`,outline:"none",fontSize:13,background:T.bg,resize:"vertical"}} onFocus={e=>e.target.style.borderColor=T.accent} onBlur={e=>e.target.style.borderColor=T.border}/></div>
            </div>
          )}
          ...
          {step===2 && (
            <div className="ai" style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div><FL label="Price/Person (CAD/mo)" req/><Inp type="number" value={form.price_per_person} onChange={e=>set("price_per_person",e.target.value)} placeholder="e.g. 1100" icon="💰" error={errs.price}/></div>
                <div><FL label="Total Rent (CAD/mo)"/><Inp type="number" value={form.total_rent} onChange={e=>set("total_rent",e.target.value)} placeholder="e.g. 2200" icon="💵"/></div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                <div><FL label="Bedrooms" req/><SI value={form.beds} onChange={e=>set("beds",e.target.value)} options={["1","2","3","4","5"]} placeholder="Beds"/>{errs.beds&&<p style={{color:T.red,fontSize:11,marginTop:4}}>Req</p>}</div>
                <div><FL label="Bathrooms" req/><SI value={form.baths} onChange={e=>set("baths",e.target.value)} options={["1","1.5","2","2.5","3"]} placeholder="Baths"/></div>
                <div><FL label="Spots" req/><SI value={form.available_spots} onChange={e=>set("available_spots",e.target.value)} options={["1","2","3","4"]} placeholder="Spots"/></div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                  {[["rent_controlled","Rent Controlled"],["pets_allowed","Pets Allowed"],["smoking_allowed","Smoking OK"],["furnished","Furnished"]].map(([k,lb]) => (
                    <button key={k} onClick={() => set(k,!form[k])} style={{padding:"10px 12px",borderRadius:T.rsm,border:`2px solid ${form[k]?T.accent:T.border}`,background:form[k]?T.accentLight:T.bg,color:form[k]?T.accent:T.sub,cursor:"pointer",fontWeight:600,fontSize:12,transition:"all .18s",textAlign:"left"}}>{lb}</button>
                  ))}
                </div>
              </div>
            </div>
          ...
          {step===3 && (
            <div className="ai">
              <p style={{color:T.muted,fontSize:12,marginBottom:14}}>Select all amenities included in rent.</p>
              <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:14}}>
                {AMENITIES.map(a => (
                  <button key={a} onClick={() => toggleAmenity(a)} style={{padding:"7px 12px",borderRadius:99,border:`2px solid ${form.amenities.includes(a)?T.accent:T.border}`,background:form.amenities.includes(a)?T.accentLight:T.white,color:form.amenities.includes(a)?T.accent:T.sub,cursor:"pointer",fontSize:12,fontWeight:600,transition:"all .18s"}}>
                    {form.amenities.includes(a)?"ok ":""}{a}
                  </button>
                ))}
              </div>
              {form.amenities.length > 0 && (
                <div style={{background:T.greenLight,border:`1px solid ${T.green}22`,borderRadius:T.rsm,padding:"10px 13px"}}>
                  <div style={{color:T.green,fontWeight:700,fontSize:12,marginBottom:5}}>{form.amenities.length} amenities selected</div>
                </div>
              )}
            </div>
          )}
        ...
        <div style={{position:"sticky",bottom:0,background:T.white,borderTop:`1px solid ${T.border}`,padding:"12px 20px",display:"flex",gap:9}}>
          {step>1 && <button onClick={()=>setStep(s=>s-1)} style={{padding:"11px 18px",borderRadius:T.rsm,background:T.white,color:T.text,border:`1.5px solid ${T.border}`,fontWeight:600,fontSize:13,cursor:"pointer",flex:"0 0 90px"}}>Back</button>}
          {step<3
            ? <PBtn ch="Continue" onClick={()=>setStep(s=>s+1)} style={{flex:1}}/>
            : <PBtn ch={saving?<Spin/>:listing?"Save Changes":"Publish Listing"} onClick={save} loading={saving} style={{flex:1}}/>
          }
        </div>
      </div>
    </div>
...
function LandlordCard({listing, onEdit, onDelete}) {
  const [del, setDel] = useState(false)
  const sqftPP = listing.sqft && listing.beds ? Math.round(listing.sqft / listing.beds) : null
  const sc = {active:{c:T.green,bg:T.greenLight,l:"Active"},draft:{c:T.gold,bg:T.goldLight,l:"Draft"},rented:{c:T.muted,bg:T.bg,l:"Rented"}}[listing.status] || {c:T.green,bg:T.greenLight,l:"Active"}
  ...
  return (
    <div className="lift" style={{background:T.white,borderRadius:T.rlg,border:`1px solid ${T.border}`,overflow:"hidden"}}>
      <div style={{height:4,background:listing.status==="active"?`linear-gradient(90deg,${T.accent},${T.green})`:listing.status==="draft"?T.gold:T.border}}/>
      <div style={{padding:"14px 15px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:9}}>
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{color:T.muted,fontSize:12}}>📍 {listing.address}{listing.city?`, ${listing.city}`:""}</div>
          </div>
          <span style={{display:"inline-flex",alignItems:"center",padding:"3px 9px",borderRadius:99,fontSize:11,fontWeight:700,color:sc.c,background:sc.bg,whiteSpace:"nowrap"}}>{sc.l}</span>
        </div>
        <div style={{display:"flex",gap:10,fontSize:11,color:T.sub,marginBottom:8,flexWrap:"wrap"}}>
          <span>{listing.beds}bd</span>
          <span>{listing.baths}ba</span>
          {sqftPP && <span>{sqftPP} sqft/p</span>}
          <span>{listing.available_spots} spot{listing.available_spots!==1?"s":""}</span>
          {listing.rent_controlled && <span style={{color:T.green,fontWeight:700}}>Rent Ctrl</span>}
        </div>
        {listing.amenities?.length>0 && (
          <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:9}}>
            {listing.amenities.slice(0,4).map(a=><span key={a} style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:5,padding:"2px 7px",fontSize:10,color:T.sub}}>{a}</span>)}
            {listing.amenities.length>4&&<span style={{fontSize:10,color:T.muted}}>+{listing.amenities.length-4}</span>}
          </div>
        )}
        <div style={{height:1,background:T.border,margin:"8px 0"}}/>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <span style={{fontSize:19,fontWeight:800}}>${listing.price_per_person?.toLocaleString()}</span>
            <span style={{color:T.muted,fontSize:11}}>/person/mo</span>
          </div>
          <div style={{display:"flex",gap:7}}>
            <PBtn ch="Edit" size="sm" onClick={()=>onEdit(listing)} style={{background:T.bg,color:T.text,border:`1px solid ${T.border}`}}/>
            <PBtn ch={del?<Spin c={T.red}/>:"Del"} size="sm" onClick={handleDelete} style={{background:T.redLight,color:T.red,border:`1px solid ${T.red}22`}}/>
          </div>
        </div>
      </div>
    </div>
...
function LandlordDashboard() {
  const {user, profile, signOut, busy} = useAuth()
  ...
  const stats = {
    ...
    revenue: listings.filter(l=>l.status==="active").reduce((s,l)=>s+(l.total_rent||0),0)
  }
  const tabs = [{id:"listings",icon:"🏠",label:"Listings"},{id:"profile",icon:"👤",label:"Profile"}]

  return (
    <div style={{minHeight:"100vh",background:T.bg,paddingBottom:80}}>
      <style>{GS}</style>
      {toast && <Toast msg={toast} onDone={()=>setToast(null)}/>}
      {showForm && <ListingFormModal listing={editL} onClose={()=>{setShowForm(false);setEditL(null)}} onSaved={()=>{fetchListings();setToast(editL?"Updated!":"Published!")}}/>}

      <nav style={{background:"rgba(249,247,244,.97)",backdropFilter:"blur(10px)",borderBottom:`1px solid ${T.border}`,padding:"0 16px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          <div style={{width:28,height:28,borderRadius:7,background:T.accent,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>🏠</div>
          <span style={{fontFamily:"'Instrument Serif',serif",fontSize:17}}>NestMatch</span>
          <span style={{display:"inline-flex",padding:"3px 9px",borderRadius:99,fontSize:11,fontWeight:700,color:T.blue,background:T.blueLight}}>Landlord</span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <PBtn ch="+ New Listing" size="sm" onClick={()=>{setEditL(null);setShowForm(true)}}/>
          <button onClick={signOut} disabled={busy} style={{padding:"6px 12px",borderRadius:T.rsm,background:"transparent",border:`1.5px solid ${T.border}`,color:T.sub,cursor:"pointer",fontWeight:600,fontSize:12}}>{busy?"..":"Sign Out"}</button>
        </div>
      </nav>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"0 13px"}}>
        {tab==="listings" && (
          <div>
            <div className="au" style={{padding:"18px 0 14px"}}>
              <div style={{fontFamily:"'Instrument Serif',serif",fontSize:22,marginBottom:2}}>Good day, {profile?.full_name?.split(" ")[0]||"there"} 👋</div>
              <p style={{color:T.muted,fontSize:13}}>Manage your listings and connect with verified renters.</p>
            </div>
            <div className="au s1" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:18}}>
              {[["🏠","Total",stats.total,T.accent,T.accentLight],["🟢","Active",stats.active,T.green,T.greenLight],["🔒","Rented",stats.rented,T.blue,T.blueLight],["💰","Revenue/mo","$"+stats.revenue.toLocaleString(),T.gold,T.goldLight]].map(([ic,lb,v,c,bg])=>(
                <div key={lb} style={{background:T.white,borderRadius:T.r,padding:"12px 10px",border:`1px solid ${T.border}`,textAlign:"center"}}>
                  <div style={{fontSize:18,marginBottom:4}}>{ic}</div>
                  <div style={{fontWeight:800,fontSize:17,color:c}}>{v}</div>
                  <div style={{color:T.muted,fontSize:10}}>{lb}</div>
                </div>
              ))}
            </div>
            <div style={{display:"flex",gap:6,marginBottom:14}}>
              {[["all","All"],["active","Active"],["draft","Draft"],["rented","Rented"]].map(([v,lb])=>(
                <button key={v} onClick={()=>setFilter(v)} style={{padding:"6px 12px",borderRadius:99,border:`1.5px solid ${filter===v?T.accent:T.border}`,background:filter===v?T.accentLight:T.white,color:filter===v?T.accent:T.sub,cursor:"pointer",fontSize:12,fontWeight:600,transition:"all .18s"}}>{lb}</button>
              ))}
            </div>
            {loadingL ? (
              <div style={{textAlign:"center",padding:"40px 20px",color:T.muted}}>
                <div style={{width:28,height:28,borderRadius:"50%",border:`3px solid ${T.accent}22`,borderTopColor:T.accent,animation:"spin .8s linear infinite",margin:"0 auto 10px"}}/>
                Loading...
              </div>
            ) : filtered.length===0 ? (
              <div className="au" style={{textAlign:"center",padding:"52px 20px",background:T.white,borderRadius:T.rlg,border:`1px solid ${T.border}`}}>
                <div style={{fontSize:44,marginBottom:12}}>🏠</div>
                <div style={{fontFamily:"'Instrument Serif',serif",fontSize:20,marginBottom:6}}>{filter==="all"?"No listings yet":"No "+filter+" listings"}</div>
                <p style={{color:T.muted,fontSize:13,marginBottom:20,lineHeight:1.6}}>{filter==="all"?"Post your first listing and start receiving verified renter applications!":""}</p>
                {filter==="all"&&<PBtn ch="+ Post Your First Listing" onClick={()=>{setEditL(null);setShowForm(true)}}/>}
              </div>
            ) : (
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
                {filtered.map((l,i) => (
                  <div key={l.id} className={"au s"+(Math.min(i+1,4))}>
                    <LandlordCard listing={l} onEdit={l=>{setEditL(l);setShowForm(true)}} onDelete={id=>{setListings(ls=>ls.filter(x=>x.id!==id));setToast("Deleted")}}/>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="ai" style={{paddingTop:18}}>
            ...
            <div style={{background:T.white,borderRadius:T.rlg,padding:20,border:`1px solid ${T.border}`}}>
              <div style={{display:"flex",gap:13,alignItems:"center",marginBottom:14}}>
                <div style={{width:60,height:60,borderRadius:"50%",background:T.blueLight,border:`3px solid ${T.blue}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26}}>🏢</div>
                <div>
                  <div style={{fontFamily:"'Instrument Serif',serif",fontSize:19}}>{profile?.full_name}</div>
                  <div style={{color:T.muted,fontSize:12,marginTop:2}}>{profile?.email}</div>
                </div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                {[["💼","Occupation",profile?.occupation||"—"],["🏠","Listings",stats.total],["💰","Revenue/mo","$"+stats.revenue.toLocaleString()],["🟢","Active",stats.active]].map(([ic,lb,v])=>(
                  <div key={lb} style={{background:T.bg,borderRadius:T.rsm,padding:"10px 11px"}}>
                    <div style={{fontSize:16,marginBottom:2}}>{ic}</div>
                    <div style={{color:T.muted,fontSize:9,fontWeight:600,textTransform:"uppercase",letterSpacing:.5}}>{lb}</div>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(249,247,244,.97)",backdropFilter:"blur(10px)",borderTop:`1px solid ${T.border}`,display:"flex",justifyContent:"space-around",padding:"8px 0 12px",zIndex:50}}>
        {tabs.map(t => (
          <button key={t.id} onClick={()=>setTab(t.id)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"none",border:"none",cursor:"pointer",color:tab===t.id?T.accent:T.muted,minWidth:60,position:"relative"}}>
...
// Renter view - shows real listings from DB
function RenterDashboard() {
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg}}>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"18px 13px"}}>
        <div className="au" style={{marginBottom:18}}>
          <div style={{fontFamily:"'Instrument Serif',serif",fontSize:22}}>Welcome back, {profile?.full_name?.split(" ")[0]}!</div>
          <p style={{color:T.muted,fontSize:13,marginTop:2}}>Browse verified listings from trusted landlords.</p>
        </div>
        {loading ? (
          <div style={{textAlign:"center",padding:40,color:T.muted}}>
            <div style={{width:28,height:28,borderRadius:"50%",border:`3px solid ${T.accent}22`,borderTopColor:T.accent,animation:"spin .8s linear infinite",margin:"0 auto 10px"}}/>Loading...
          </div>
        ) : listings.length===0 ? (
          <div style={{textAlign:"center",padding:"52px 20px",background:T.white,borderRadius:T.rlg,border:`1px solid ${T.border}`}}>
            <div style={{fontSize:40,marginBottom:12}}>🏠</div>
            <div style={{fontFamily:"'Instrument Serif',serif",fontSize:18,marginBottom:6}}>No listings yet</div>
            <p style={{color:T.muted,fontSize:13,lineHeight:1.6}}>Landlords are setting up. Check back soon!</p>
          </div>
        ) : (
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
            {listings.map((l,i) => (
              <div key={l.id} className={"au s"+(Math.min(i+1,4))} style={{background:T.white,borderRadius:T.rlg,border:`1px solid ${T.border}`,overflow:"hidden"}}>
                <div style={{height:4,background:`linear-gradient(90deg,${T.accent},${T.green})`}}/>
                <div style={{padding:"13px 14px"}}>
                  <div style={{fontWeight:700,fontSize:14,marginBottom:1}}>{l.title}</div>
                  <div style={{color:T.muted,fontSize:12,marginBottom:8}}>📍 {l.city}</div>
                  <div style={{display:"flex",gap:10,fontSize:11,color:T.sub,marginBottom:8,flexWrap:"wrap"}}>
                    <span>{l.beds}bd</span><span>{l.baths}ba</span>
                    {l.sqft&&l.beds&&<span>{Math.round(l.sqft/l.beds)} sqft/p</span>}
                    {l.rent_controlled&&<span style={{color:T.green,fontWeight:700}}>Rent Controlled</span>}
                  </div>
                  {l.amenities?.length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>{l.amenities.slice(0,3).map(a=><span key={a} style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:4,padding:"2px 7px",fontSize:10,color:T.sub}}>{a}</span>)}</div>}
                  <div style={{height:1,background:T.border,margin:"8px 0"}}/>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div><span style={{fontSize:18,fontWeight:800}}>${l.price_per_person?.toLocaleString()}</span><span style={{color:T.muted,fontSize:11}}>/person/mo</span></div>
                    <PBtn ch="View" size="sm"/>
                  </div>
                </div>
              </div>
            ))}
...
function AppShell(){
  const{isAuth,loading,onboardingDone,isLandlord}=useAuth()
  if(loading)return(
